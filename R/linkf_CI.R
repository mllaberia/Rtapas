...
#'
#' From the matrix obtained in \code{\link[=prob_statics]{prob_statics()}},
...
linkf_CI <- function (freqfun = "geo_D", x, fx, c.level = 95, barplot = TRUE,
  ...
  if(freqfun == "geo_D") {
    ...
    colnames(df) <- c("Taxa1", "Taxa2", "GD.wFq", "GD.LO", "GD.HI", "GD.AV")
  ...
  if(freqfun == "paco_ss") {
    ...
    df <- data.frame(LFPACO01[,1], LFPACO01[,2], LFPACO01[, ncol(LFPACO01)],
  ...
  if(freqfun == "paraF") {
    ...
    if(barplot == TRUE) {
      link.fq <- barplot(PF.AV, xaxt = 'n', horiz = FALSE, cex.names = 0.6,
                         las=2, cex.axis=0.8,
                         ylab="Observed - Expected frequency",
                         ylim=c(min(PF.LO), max(PF.HI)), col = col.bar, ...)
      suppressWarnings(arrows(link.fq, PF.HI, link.fq, PF.LO, length= 0,
                              angle=90, code=3, col = col.ci))
      axis(side=1, at=link.fq[1:length(PF.AV)], labels=LFPF01$HS, las=2,
           tick = FALSE, line= 0.1, cex.axis=0.5)
